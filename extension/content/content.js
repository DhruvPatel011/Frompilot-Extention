...
(function () {
  ...
  function getLabel(el) {
    ...
    if (wl) {
      ...
      cl.querySelectorAll("input,select,textarea,button").forEach((e) =>
        e.remove(),
      );
  ...
  function simInput(el, val) {
    ...
    const ts = Object.getOwnPropertyDescriptor(
    ...
    for (let pass = 0; pass < 4; pass++) {
      for (const opt of el.options) {
        const ov = opt.value.toLowerCase(),
          ot = opt.text.toLowerCase().trim();
        if (
          [
            ov === v,
            ot === v,
            ot.startsWith(v) || ov.startsWith(v),
            ot.includes(v) || v.includes(ot),
          ][pass]
        ) {
          el.value = opt.value;
  ...
  function extractAllFields() {
    ...
    for (const [name, cbs] of cbByName) {
      ...
      if (cbs.length === 1) {
        ...
        fields.push({
          ...
          placeholder: "",
          required:
            first.required || first.getAttribute("aria-required") === "true",
          name: first.name || "",
          id: first.id || "",
          currentValue: isChecked(first) ? "yes" : "no",
        ...
        fields.push({
          ...
          placeholder: "",
          required: cbs.some(
            (cb) => cb.required || cb.getAttribute("aria-required") === "true",
          ),
          name: first.name || "",
          id: first.id || "",
          currentValue: cbs
            .filter(isChecked)
            .map((cb) => getLabel(cb) || cb.value)
            .join(", "),
          fillState: FS.DETECTED,
        });
      }
    ...
    for (const grp of unnamedGroups) {
      ...
      const first = grp[0];
      const groupTitle =
        findGroupTitle(first) || getLabel(first) || "Checkbox Group";
    ...
    const others = Array.from(document.querySelectorAll(SELECTORS)).filter(
      isVisible,
    );
    for (const el of others) {
      ...
      if (
        ["submit", "button", "reset", "hidden", "file", "image"].includes(type)
      )
        continue;
      let fieldType = "text";
  ...
  async function fillField(field, value, delay) {
    ...
    try {
      // SELECT
      if (type === "select")
        return fillSelect(el, value) ? FS.FILLED : FS.SKIPPED;
  ...
  const obs = new MutationObserver((muts) => {
    const changed = muts.some((m) =>
      Array.from(m.addedNodes).some(
        (n) =>
          n.nodeType === 1 &&
          (n.matches?.("form,input,select,textarea") ||
  ...
  async function handleMsg(msg) {
    ...
    switch (type) {
      ...
      case "FILL_FORM": {
        ...
        chrome.runtime
          .sendMessage({ type: "FORM_FILLED", payload: { filled } })
          .catch(() => {});
        return {
          success: true,
          filled,
