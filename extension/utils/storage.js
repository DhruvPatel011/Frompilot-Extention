...
(function () {
  ...
  var FPStorage = {
    ...
    setTokens: function (accessToken, refreshToken) {
      var expiry = null;
      try {
        expiry = JSON.parse(atob(accessToken.split(".")[1])).exp * 1000;
      } catch (e) {}
      var d = {};
    ...
    getAccessToken: async function () {
      ...
      var t = r[KEYS.ACCESS_TOKEN],
        exp = r[KEYS.TOKEN_EXPIRY];
      if (!t) return null;
      if (exp && Date.now() >= exp) {
        await FPStorage.clearTokens();
        return null;
      }
      return t;
    },
    ...
    updateField: async function (profileId, fieldId, updates) {
      ...
      if (!p) return null;
      var f = p.fields.find(function (f) {
        return f.id === fieldId;
      });
      if (!f) return null;
    ...
    deleteField: async function (profileId, fieldId) {
      ...
      if (!p) return;
      p.fields = p.fields.filter(function (f) {
        return f.id !== fieldId;
      });
      return FPStorage.saveProfiles(profiles);
    },
    ...
    addHistoryEntry: async function (entry) {
      ...
      history.unshift(
        Object.assign({ id: "h_" + Date.now(), createdAt: Date.now() }, entry),
      );
      if (history.length > 100) history = history.slice(0, 100);
      var d = {};
      d[KEYS.HISTORY] = history;
      return chrome.storage.local.set(d);
    },
