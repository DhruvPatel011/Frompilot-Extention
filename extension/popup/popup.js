...
function apiProxy(method, endpoint, body) {
  return new Promise(function (resolve, reject) {
    chrome.runtime.sendMessage(
      {
        type: "API_REQUEST",
        payload: { method: method, endpoint: endpoint, body: body || null },
      },
      function (res) {
        ...
        if (res && res.success) resolve(res.data);
          ...
          reject(
            Object.assign(new Error((res && res.error) || "Request failed"), {
...
function findProfile(pid) {
  return S.profiles.find(function (p) {
    return getPid(p) === pid;
  });
}
...
function getProfileKV(pid) {
  ...
  return p.fields.map(function (f) {
    return { key: f.key, value: f.value, aliases: f.aliases || [] };
  });
}
...
function renderProfileSelect() {
  ...
  if (!S.profiles.length) {
    sel.innerHTML =
      '<option value="">No profiles — create one in Profiles tab</option>';
    return;
  }
  sel.innerHTML = S.profiles
    .map(function (p) {
      return (
        ...
        esc(getPid(p)) +
        '"' +
        (p.isDefault ? " selected" : "") +
        ">" +
        esc(p.name || p.profileName) +
        (p.isDefault ? " ★" : "") +
        "</option>"
      );
    })
...
async function detectForm() {
  ...
  try {
    ...
    if (res && res.hasForm) {
      ...
      if (txt)
        txt.textContent = "Form found — " + res.fieldCount + " fields detected";
...
async function fetchFields(tabId) {
  try {
    ...
    if (res && res.success && res.fields && res.fields.length) {
      ...
      show($("fieldsInfo"));
      var fb = $("fillBtn");
      if (fb) fb.removeAttribute("disabled");
      if (S.settings.highlightFields !== false) {
        chrome.tabs
          .sendMessage(tabId, { type: "HIGHLIGHT_FIELDS" })
          .catch(function () {});
      }
    }
  } catch (e) {}
...
function setStats(total, req, filled, missing) {
  ...
  if (r) r.textContent = req;
  if (f) f.textContent = filled;
  if (m) m.textContent = missing;
}
...
async function handleFill() {
  if (!S.detectedFields.length) {
    toast("No fields detected. Click Analyze Form first.", "error");
    return;
  }
  var btn = $("fillBtn"),
    bt = $("fillBtnText");
  ...
  try {
    ...
    if (S.geminiKey) {
      try {
        ...
        S.aiFilled = Object.values(aiAns).filter(function (v) {
          return v && v !== "";
        }).length;
      } catch (err) {
    ...
    var fillRes = await chrome.tabs.sendMessage(tab.id, {
      ...
      payload: {
        answers: answers,
        delay: S.settings.fillDelay || 80,
        highlightFields: S.settings.highlightFields !== false,
      },
    });
    ...
    setStats(
      total,
      S.detectedFields.filter(function (f) {
        return f.required;
      }).length,
      total - missing,
      missing,
    ...
    FPStorage.addHistoryEntry({
      ...
      totalFields: total,
      filledFields: filled,
      status: missing === 0 ? "completed" : "partial",
    }).catch(function () {});
    ...
    var b = $("formStatusBanner"),
      t = $("formStatusText");
    if (b)
      b.className =
        filled === total ? "status-banner success" : "status-banner found";
    if (t)
      t.textContent =
        (filled === total ? "✓ " : "") +
        " Filled " +
...
function localMatch(kv) {
  var answers = {},
    threshold = (S.settings.fuzzyThreshold || 45) / 100;
...
function showReport(total, ai, user, manual) {
  ...
  if (ra) ra.textContent = ai;
  if (ru) ru.textContent = user;
  if (rm) rm.textContent = manual;
...
function initWizard(answers, results) {
  wizard.fields = S.detectedFields
    ...
    .filter(function (x) {
      var hasAnswer =
        answers[x.i] !== undefined && String(answers[x.i]).trim() !== "";
...
function renderWizardStep() {
  ...
  var typeBadge = "";
  if (f.type === "select")
    typeBadge = '<span class="mf-type-badge">SELECT</span>';
  else if (f.type === "radio_group" || f.type === "radio")
    typeBadge = '<span class="mf-type-badge">CHOICE</span>';
  else if (f.type === "checkbox_group")
    typeBadge = '<span class="mf-type-badge">CHECKBOX</span>';
  else if (f.type === "date")
    typeBadge = '<span class="mf-type-badge">DATE</span>';
  ...
  if (isOptionType && f.options && f.options.length) {
    ...
    optionsHTML = f.options
      .map(function (o, oi) {
        ...
        return (
          '<div class="wiz-option-item" data-index="' +
          oi +
          '" data-value="' +
          esc(o.label || o.value) +
          '" data-type="' +
          listType +
          '" style="display:flex;align-items:center;gap:10px;padding:8px 11px;margin-bottom:4px;border:1.5px solid var(--border);border-radius:6px;background:var(--bg-card);cursor:pointer;font-size:13px;font-weight:500;color:var(--text);">' +
          ind +
          "<span>" +
          esc(o.label || o.value) +
          "</span>" +
  ...
  list.innerHTML =
    ...
    '<div style="width:' +
    pct +
    '%;height:100%;background:var(--accent);border-radius:3px;transition:width .3s;"></div>' +
    "</div>" +
    ...
    (isOptionType && optionsHTML
      ? '<div id="wizMainInput" data-type="' +
        (f.type === "checkbox_group" ? "checkbox" : "radio") +
        '" style="display:flex;flex-direction:column;gap:4px;outline:none;">' +
        optionsHTML +
        "</div>"
      : buildWizardInput(f)) +
    "</div>" +
...
function buildWizardInput(f) {
  ...
  if (
    ...
    return (
      '<div class="wiz-option-list" id="wizMainInput" data-type="' +
      listType +
      '">' +
      opts
        .map(function (o, i) {
  ...
  if (type === "checkbox") {
    return (
      '<div class="wiz-option-list" id="wizMainInput" data-type="radio">' +
  ...
  if (
    ...
    return (
      '<input type="' +
      type +
      '" class="wiz-text-input" id="wizMainInput" autocomplete="off">'
    );
  }
  ...
  return (
    '<input type="' +
    it +
    '" class="wiz-text-input" id="wizMainInput" placeholder="' +
    esc(ph) +
    '" autocomplete="off">'
  );
}
...
function setupWizardKeyboard(f) {
  ...
  var optList = area.querySelector(
    "#wizMainInput[data-type], .wiz-option-list",
  );
  ...
  if (optList && optList.querySelector(".wiz-option-item")) {
    ...
    optList.addEventListener("keydown", function (e) {
      if (e.key === "ArrowDown" || e.key === "Tab") {
        ...
        e.preventDefault();
        toggleItem(items[focusIdx]);
      } else if (e.key === "Enter") {
        e.preventDefault();
        var anySelected = items.some(function (it) {
          return it.dataset.selected === "1";
        });
...
function getWizardValue() {
  ...
  if (optList && optList.querySelector(".wiz-option-item")) {
    ...
    var selected = items.filter(function (it) {
      return (
        it.dataset.selected === "1" || it.classList.contains("wiz-selected")
      );
    });
    if (listType === "radio")
      return selected.length ? selected[0].dataset.value : "";
    return selected
      .map(function (s) {
        return s.dataset.value;
      })
      .join(",");
  }
...
function getMFValue(row, i) {
  return "";
}

function showSavePrompt() {
  var n = S.pendingSave.length;
  if (!n) return;
  var t = $("savePromptText");
  if (t)
    t.textContent =
      "Save " + n + " value" + (n > 1 ? "s" : "") + " to profile?";
  show($("savePrompt"));
}
...
function renderProfileList() {
  ...
  if (!S.profiles.length) {
    list.innerHTML =
      '<div class="empty-state"><p>No profiles yet.<br>Click "+ New" to create one.</p></div>';
    hide($("kvEditorWrap"));
    return;
  }
  list.innerHTML = S.profiles
    .map(function (p) {
      ...
      return (
        ...
        '<div class="p-actions">' +
        (!p.isDefault
          ? '<button class="btn-icon accent p-btn" data-action="default" data-pid="' +
            esc(pid) +
            '" title="Set default">★</button>'
          : "") +
        '<button class="btn-icon p-btn" data-action="rename" data-pid="' +
        esc(pid) +
        '" title="Rename">✏</button>' +
        '<button class="btn-icon danger p-btn" data-action="delete" data-pid="' +
        esc(pid) +
        '" title="Delete">✕</button>' +
        "</div>" +
        "</div>"
...
async function onProfileAction(action, pid) {
  if (action === "delete") {
    ...
    if (!S.isGuest) {
      try {
        await apiProxy("DELETE", "/api/profiles/" + pid);
      } catch (e) {}
    }
    if (S.activePid === pid) {
      S.activePid = null;
  ...
  if (action === "open") {
    if (S.activePid === pid) {
      ...
      S.kvOpen = false;
      hide($("kvEditorWrap"));
      $$(".profile-card").forEach(function (c) {
      ...
      S.kvOpen = true;
      $$(".profile-card").forEach(function (c) {
        c.classList.toggle("active-card", c.dataset.pid === pid);
      });
      show($("kvEditorWrap"));
      renderKVEditor(pid);
    }
...
function renderKVEditor(pid) {
  var p = findProfile(pid);
  if (!p) return;
  var nameEl = $("kvProfileName");
  if (nameEl) nameEl.textContent = (p.name || "Profile") + " Fields";
  renderCatChips();
  renderKVList(pid);
}
...
function renderCatChips() {
  ...
  row.innerHTML =
    '<span class="cat-chip' +
    (S.kvFilter === "all" ? " active" : "") +
    '" data-cat="all">All</span>' +
    S.categories
      .map(function (c) {
        return (
          '<span class="cat-chip' +
...
function renderKVList(pid) {
  ...
  var countEl = $("kvCount");
  if (countEl)
    countEl.textContent = filtered.length + " of " + fields.length + " fields";
  var list = $("kvList");
  if (!list) return;
  if (!filtered.length) {
    list.innerHTML =
      ...
      (S.kvFilter !== "all"
        ...
        : "No fields yet.") +
      "<br>Use the form above to add one.</p></div>";
    return;
  }
  list.innerHTML = filtered
    .map(function (f) {
      ...
      return (
        ...
        esc(f.id) +
        '">' +
        '<input type="text" class="kv-cell key kv-key" data-fid="' +
        esc(f.id) +
        '" value="' +
        esc(f.key) +
        '" placeholder="Key">' +
        '<input type="text" class="kv-cell kv-val" data-fid="' +
        esc(f.id) +
        '" value="' +
        esc(f.value) +
        '" placeholder="Value">' +
        '<div class="kv-row-acts">' +
        '<input type="checkbox" class="kv-cb" data-fid="' +
        esc(f.id) +
        '"' +
        (sel ? " checked" : "") +
        ' title="Select">' +
        '<button class="btn-icon danger kv-del" data-fid="' +
        esc(f.id) +
        '" title="Delete field" style="padding:3px">' +
...
async function addField() {
  ...
  var meta = document.querySelector(
    '.profile-card[data-pid="' + esc(S.activePid) + '"] .p-meta',
  );
  if (meta && p)
    meta.innerHTML =
      (p.fields ? p.fields.length : 0) +
...
function openProfileModal(pid) {
  ...
  if (title) title.textContent = isNew ? "New Profile" : "Rename Profile";
...
async function saveProfile() {
  ...
  try {
    if (isNew) {
      ...
      if (!S.isGuest) {
        try {
          await apiProxy("PUT", "/api/profiles/" + pid, { profileName: name });
        } catch (e) {}
      }
...
function exportProfile() {
  if (!S.activePid) {
    toast("Open a profile first", "error");
    return;
  }
  FPStorage.exportProfile(S.activePid).then(function (json) {
    ...
    a.href = u;
    a.download = "formpilot-profile.json";
    a.click();
...
async function loadHistory(search) {
  ...
  if (search) {
    ...
    entries = entries.filter(function (e) {
      return (
        (e.formTitle || "").toLowerCase().includes(q) ||
  ...
  if (!S.isGuest) {
    try {
      var res = await apiProxy(
        "GET",
        "/api/forms/history?limit=30" +
          (search ? "&search=" + encodeURIComponent(search) : ""),
      );
      var remote = (res && res.data && res.data.forms) || [];
      var us = new Set(
        remote.map(function (r) {
          return r.formUrl;
        }),
      );
      entries = remote.concat(
        entries.filter(function (e) {
          return !us.has(e.formUrl);
        }),
      );
    } catch (e) {}
  }
  ...
  list.innerHTML = entries
    ...
    .map(function (e) {
      ...
      return (
        ...
        esc(e.formTitle || e.formUrl || "Untitled") +
        "</div>" +
        ...
        s +
        "</span></div>" +
        '<div class="history-meta" style="margin-top:2px"><span>' +
        fmtDate(e.createdAt) +
        "</span><span>" +
        (e.filledFields || 0) +
        "/" +
...
async function loadStats() {
  ...
  var total = entries.length,
    done = entries.filter(function (e) {
  ...
  var totalF = entries.reduce(function (a, e) {
    return a + (e.filledFields || 0);
  }, 0);
  c.innerHTML =
    '<div class="stat-card"><div class="stat-val">' +
    total +
    '</div><div class="stat-lbl">Total Forms</div></div>' +
    '<div class="stat-card"><div class="stat-val grn">' +
    rate +
    '%</div><div class="stat-lbl">Success Rate</div></div>' +
    '<div class="stat-card"><div class="stat-val acc">' +
    thisMonth +
    '</div><div class="stat-lbl">This Month</div></div>' +
    '<div class="stat-card"><div class="stat-val">' +
    S.profiles.length +
    '</div><div class="stat-lbl">Profiles</div></div>' +
    '<div class="stat-card wide"><div class="stat-lbl">Fields Filled All Time</div><div class="stat-val acc">' +
    totalF +
    "</div></div>";
}
...
async function initSettings() {
  ...
  var ki = $("geminiKeyInput");
  if (ki) ki.value = S.geminiKey ? "•".repeat(24) : "";
  var ks = $("geminiKeyStatus");
  if (ks) {
    ks.textContent = S.geminiKey
      ? "✓ API key saved"
      : "No key — profile matching only";
    ks.style.color = S.geminiKey ? "var(--green)" : "var(--text-3)";
  }
  var fds = $("fillDelaySlider"),
    fdv = $("fillDelayVal");
  if (fds) {
    fds.value = S.settings.fillDelay || 80;
    if (fdv) fdv.textContent = (S.settings.fillDelay || 80) + "ms";
  }
  var ht = $("highlightToggle");
  if (ht) ht.checked = S.settings.highlightFields !== false;
  var sm = $("saveManualToggle");
  if (sm) sm.checked = S.settings.saveManualEntries !== false;
}
...
async function clearAllData() {
  if (
    !confirm(
      "DELETE ALL DATA?\n\nAll profiles, fields, and history will be permanently deleted.",
    )
  )
    return;
...
async function cleanOldBackendProfiles() {
  ...
  if (hasOld) {
    var clean = profiles.filter(function (p) {
      return Array.isArray(p.fields) && !p._id;
    });
...
function initEvents() {
  ...
  if (lf)
    lf.addEventListener("submit", async function (e) {
      ...
      setBtn(btn, true);
      try {
        var res = await apiProxy("POST", "/api/auth/login", {
          email: $("loginEmail").value.trim(),
          password: $("loginPassword").value,
        });
        var d = res.data || res;
        if (!d.accessToken) throw new Error("No token");
        await saveAuthData(d);
        S.user = d.user;
        await initApp(false);
      } catch (ex) {
        show(err);
  ...
  if (rf)
    rf.addEventListener("submit", async function (e) {
      ...
      if (pw.length < 8 || !/[A-Z]/.test(pw) || !/\d/.test(pw)) {
        show(err);
        if (err) err.textContent = "Password: 8+ chars, 1 uppercase, 1 number";
        return;
      }
      setBtn(btn, true);
      try {
        var res = await apiProxy("POST", "/api/auth/register", {
          name: $("regName").value.trim(),
          email: $("regEmail").value.trim(),
          password: pw,
        });
        var d = res.data || res;
        if (!d.accessToken) throw new Error("Registration failed");
        await saveAuthData(d);
        S.user = d.user;
        await initApp(false);
      } catch (ex) {
        show(err);
        if (err) err.textContent = ex.message || "Register failed";
      } finally {
        setBtn(btn, false);
      }
    });
  ...
  if (tb)
    tb.addEventListener("click", function () {
      var order = ["light", "dark", "system"],
        cur = S.settings.theme || "light";
      var next = order[(order.indexOf(cur) + 1) % order.length];
      applyTheme(next);
      FPStorage.saveSettings({ theme: next });
  ...
  document.addEventListener("click", function (e) {
    ...
    if (to) {
      applyTheme(to.dataset.themeOpt);
      FPStorage.saveSettings({ theme: to.dataset.themeOpt });
    }
  });
  ...
  if (ab)
    ab.addEventListener("click", async function () {
      ab.disabled = true;
      ab.textContent = "Analyzing…";
      S.detectedFields = [];
      hide($("fieldsInfo"));
      hide($("completionReport"));
      hide($("manualPanel"));
      var fb = $("fillBtn");
      if (fb) fb.setAttribute("disabled", "");
      try {
        await detectForm();
      } finally {
        ab.disabled = false;
        ab.innerHTML =
          '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg> Analyze Form';
      }
    });

  /* FILL */
  var fb = $("fillBtn");
  if (fb) fb.addEventListener("click", handleFill);

  /* METHOD BUTTONS */
  $$(".method-btn").forEach(function (btn) {
    btn.addEventListener("click", function () {
      $$(".method-btn").forEach(function (b) {
        b.classList.remove("active");
      });
      btn.classList.add("active");
      S.fillMethod = btn.dataset.method;
      var labels = {
        ai: "Fill with AI",
        profile: "Fill from Profile",
        job: "Fill Job App",
        scholarship: "Fill Scholarship",
      };
      var t = $("fillBtnText");
      if (t) t.textContent = labels[S.fillMethod] || "Fill Form";
    });
  });
  ...
  if (profileTab) {
    profileTab.addEventListener("click", async function (e) {
      ...
      if (card) {
        await onProfileAction("open", card.dataset.pid);
        return;
      }
      // Category chips
      ...
      if (chip) {
        ...
        $$(".cat-chip").forEach(function (c) {
          c.classList.toggle("active", c.dataset.cat === S.kvFilter);
        });
    ...
    profileTab.addEventListener("focusout", async function (e) {
      var cell = e.target.closest(".kv-key,.kv-val");
      if (!cell || !S.activePid) return;
      var fid = cell.dataset.fid,
        isKey = cell.classList.contains("kv-key"),
        val = cell.value.trim();
      if (isKey && !val) {
        cell.value = "Key";
        return;
      }
      await FPStorage.updateField(
        S.activePid,
        fid,
        isKey ? { key: val } : { value: val },
      );
      S.profiles = await FPStorage.getProfiles();
    });
  }
  ...
  /* NEW PROFILE */
  var npb = $("newProfileBtn");
  if (npb)
    npb.addEventListener("click", function () {
      openProfileModal(null);
    });
  var spb = $("saveProfileBtn");
  ...
  var mpn = $("mpName");
  if (mpn)
    mpn.addEventListener("keydown", function (e) {
      if (e.key === "Enter") saveProfile();
    });

  /* EXPORT / IMPORT */
  var exb = $("exportProfileBtn");
  if (exb) exb.addEventListener("click", exportProfile);
  var imb = $("importProfileBtn");
  if (imb) imb.addEventListener("click", importProfile);

  /* ADD FIELD */
  var afb = $("addFieldBtn");
  if (afb) afb.addEventListener("click", addField);
  var nfv = $("newFieldVal");
  if (nfv)
    nfv.addEventListener("keydown", function (e) {
      if (e.key === "Enter") addField();
    });
  var nfk = $("newFieldKey");
  if (nfk)
    nfk.addEventListener("keydown", function (e) {
  ...
  if (bdb)
    bdb.addEventListener("click", async function () {
      if (!S.selectedFids.size) {
        toast("Select fields first", "error");
        return;
      }
  ...
  if (ch)
    ch.addEventListener("click", async function () {
      ...
      if (!S.isGuest) {
        try {
          await apiProxy("DELETE", "/api/forms/history");
        } catch (e) {}
      }
      toast("Cleared", "info");
      loadHistory();
  ...
  if (sgb)
    sgb.addEventListener("click", async function () {
      var inp = $("geminiKeyInput"),
        key = (inp && inp.value.trim()) || "";
      if (key && key.replace(/•/g, "").length === 0) {
        toast("Enter a real API key", "error");
        return;
      }
      S.geminiKey = key;
      ...
      var ks = $("geminiKeyStatus");
      if (ks) {
        ks.textContent = key ? "✓ Saved!" : "Key removed";
        ks.style.color = key ? "var(--green)" : "var(--text-3)";
      }
  ...
  var fds = $("fillDelaySlider"),
    fdv = $("fillDelayVal");
  if (fds)
    fds.addEventListener("input", function () {
      S.settings.fillDelay = parseInt(fds.value);
      if (fdv) fdv.textContent = fds.value + "ms";
  ...
  if (htg)
    htg.addEventListener("change", function () {
      S.settings.highlightFields = htg.checked;
      FPStorage.saveSettings({ highlightFields: htg.checked });
    });
  var smt = $("saveManualToggle");
  if (smt)
    smt.addEventListener("change", function () {
      S.settings.saveManualEntries = smt.checked;
      FPStorage.saveSettings({ saveManualEntries: smt.checked });
    });

  /* CLEAR ALL DATA */
  var cad = $("clearAllDataBtn");
  if (cad) cad.addEventListener("click", clearAllData);

  /* MANUAL FILL: go to field + fill one */
  document.addEventListener("click", async function (e) {
    var go = e.target.closest(".mf-go");
    if (go) {
      var idx = parseInt(go.dataset.mfGoto),
        tab = await getTab();
      if (tab)
        chrome.runtime.sendMessage({
          type: "SCROLL_TO_FIELD",
          payload: { tabId: tab.id, fieldIndex: idx },
        });
      return;
    }
    ...
    if (fo) {
      ...
      var mfRow = document.querySelector(
        '.missing-field[data-index="' + idx2 + '"]',
      );
      var val = mfRow ? getMFValue(mfRow, idx2) : "";
      if (!val) {
        toast("Enter or select a value first", "error");
        return;
      }
      var tab2 = await getTab();
      if (!tab2) return;
      try {
        var res = await chrome.tabs.sendMessage(tab2.id, {
          type: "FILL_SINGLE",
          payload: { fieldIndex: idx2, value: val, delay: 50 },
        });
        if (res && res.state === "filled") {
          S.userFilled++;
          if (mfRow) mfRow.remove();
          if (S.settings.saveManualEntries && S.detectedFields[idx2]) {
            S.pendingSave.push({
              key: S.detectedFields[idx2].label,
              value: val,
            });
            showSavePrompt();
          }
          ...
          toast("Could not fill this field", "error");
        }
      } catch (err) {
  ...
  var sy = $("saveYesBtn");
  if (sy)
    sy.addEventListener("click", async function () {
      ...
      S.activePid = pid;
      for (var i = 0; i < S.pendingSave.length; i++)
        await FPStorage.addField(
          pid,
          S.pendingSave[i].key,
          S.pendingSave[i].value,
          [],
        );
      S.pendingSave = [];
      hide($("savePrompt"));
